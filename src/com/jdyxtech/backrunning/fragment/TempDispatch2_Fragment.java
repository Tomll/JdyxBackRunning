 ...
 *
 */
public class TempDispatch2_Fragment extends Fragment implements OnClickListener{
	private View view;
	...
	public Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			}else if (4 == msg.arg1) { // 表示这是 落下车位锁 的网络请求返回的msg
				progressDialog.cancel();
				try {
					JSONObject jsonObject = new JSONObject((String) msg.obj);
					if (200 == jsonObject.getInt("status")) { //车门打开后，携带tempDispatchDetilData对象，跳到tempDispatch2_Fragment
						Toast.makeText(getActivity(), "车位锁已落下！", Toast.LENGTH_LONG).show();
					} else {
						Toast.makeText(getActivity(), "落车位锁失败，请重试！", Toast.LENGTH_LONG).show();
					}
				} catch (JSONException e) {
					e.printStackTrace();
				}
			}else if (5 == msg.arg1) { // 表示这是通知：更新倒计时 的msg 
				textView5.setText("任务剩余时间："+minutes+"分"+second+"秒");
			}else if (6 == msg.arg1) { // 表示这是 提交“完成任务” 的网络请求返回的msg 
				try {
					JSONObject jsonObject = new JSONObject((String) msg.obj);
					if (200 == jsonObject.getInt("status")) { // 车门打开后，携带tempDispatchDetilData对象，跳到tempDispatch2_Fragment
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		view = inflater.inflate(R.layout.fragment_temporary_dispatch2, container, false);
		initView();
		initDispatchData();
		return view;
	}
	...
	private void initView() {
		...
		// "返回"、"导航"、“开车门”、“锁车门”、“关车窗”、“落车位锁”、“完成任务”按钮 并 设置点击监听
		...
		view.findViewById(R.id.switch_l4).setOnClickListener(this);
	...
	private void adaptiveData() throws ParseException {
		...
		textView1.setText("目标网点车位\n\n"+tempDispatchDetilData.getTo_title()+tempDispatchDetilData.getTo_parknum()+"车位\n"+tempDispatchDetilData.getTo_addr());
		...
		bd09_To_Gcj02 = bd09_To_Gcj02(tempDispatchDetilData.getTo_gps().get(1), tempDispatchDetilData.getTo_gps().get(0));
	}
	...
	public void onClick(View v) {
		...
		case R.id.button_back: // 返回键（从调度任务详情_1 fragment 返回到 临时任务列表 fragment）
			getActivity().getFragmentManager().beginTransaction().replace(R.id.homeFragmentLayout, new TempDispatch1_Fragment()).commit();
			break;
		...
		case R.id.switch_l1: // 开车门
			...
			alertDialog.setMessage("确定打开车门？").setNegativeButton("取消", null).setPositiveButton("确定", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int whichButton) {
					...
					progressDialog.show();
					NetWorkUtils.controlCarAndDevice(tempDispatchDetilData.getTid(), 1, 2, handler, 1);
				}
			}).create().show();
			break;
		case R.id.switch_l2: // 锁车门
			...
			alertDialog.setMessage("确定锁上车门？").setNegativeButton("取消", null).setPositiveButton("确定", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int whichButton) {
					...
					progressDialog.show();
					NetWorkUtils.controlCarAndDevice(tempDispatchDetilData.getTid(), 1, 1, handler, 2);
				}
			}).create().show();
			break;
		case R.id.switch_l3: // 关车窗
			...
			break;
		case R.id.switch_l4: // 落车位锁
			//展示alertDialog，并设置积极、消极 的按钮
			alertDialog.setMessage("确定落下车位锁？").setNegativeButton("取消", null).setPositiveButton("确定", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int whichButton) {
					progressDialog.setMessage("正在落下车位锁");
					progressDialog.show();
					NetWorkUtils.controlCarAndDevice(tempDispatchDetilData.getTid(), 2, 1, handler, 4);
				}
			}).create().show();
			break;
		case R.id.button_complete_task: // 完成任务
			//展示alertDialog，并设置积极、消极 的按钮
			alertDialog.setMessage("是否完成任务？").setNegativeButton("取消", null).setPositiveButton("确定", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int whichButton) {
					try {
						JSONObject jsonObject = new JSONObject();
						jsonObject.put("task_id", MainActivity.sp.getString("task_id", ""));
						jsonObject.put("description", null); //任务报告：现场协助任务需要提交该数据，车辆调度任务不需要提交该数据
						String json = jsonObject.toString();
						//调用 二次封装好的okhttp异步post网络请求
						NetWorkUtils.okhttpAsyncPost(NetWorkUtils.HOST+"/clientapi/client/temptask", json, handler,6); //最后的参数：请求码（requestCode），用来区分多个网络请求
					} catch (JSONException e) {
						e.printStackTrace();
					}
				}
	...
	//**************************************** 以下为百度导航 （驾车）************************************** 
		...
		private void routeplanToNavi() {
			...
			BNRoutePlanNode eNode = new BNRoutePlanNode(bd09_To_Gcj02[0], bd09_To_Gcj02[1],"目标车位", null,CoordinateType.GCJ02);
